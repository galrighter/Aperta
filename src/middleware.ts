 ...
 *
 * מה שהדוח הבא **לא** אומר: «הדף מפנה לכתובת אתר אחרת» עם שתי הדוגמאות
 * `https://www.aperta-designs.com/` ו-`http://www.aperta-designs.com/` (17–18.8)
 * אינו תקלה — הוא ההוכחה שהשורות כאן עובדות. «דף מפנה» הוא סיבת **הרחקה**
 * מהאינדקס ולא שגיאה, וזו בדיוק המטרה: יש עמוד בית אחד, בכתובת אחת. אין שם מה
 * לתקן, ואין ללחוץ «אימות» — האימות נבדק בזחילה חוזרת שמצפה שהכתובת תפסיק
 * להפנות, והפניית origin קנוני לא תפסיק לעולם, ולכן הוא ייכשל בכל פעם.
 * מה שכן היה תקלה: אותו דוח עם דוגמה שאינה `www`, או `www` שמחזיר 200.
 * הפירוט והמדידה החיה ב-`docs/SITE.md`.
 *
