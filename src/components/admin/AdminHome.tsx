...
//
// המספרים נטענים מהמסלולים הקיימים ולא ממסלול סיכום חדש: שלוש קריאות קלות
// שקורות פעם אחת עדיפות על נקודת קצה נוספת שצריך לתחזק, ומספר שנכשל בטעינה
// מוצג כ-"—" ולא מוריד את המסך.
...
type Counts = { orders: number | null; inquiries: number | null; designs: number | null };

export default function AdminHome() {
  const [counts, setCounts] = useState<Counts>({ orders: null, inquiries: null, designs: null });
  ...
  useEffect(() => {
    ...
    void (async () => {
      const [orders, inquiries, designs] = await Promise.all([
      ...
      setCounts({ orders, inquiries, designs });
    })();
  }, []);
