...
export const he = {
   ...
   */
  errContentBlocked:
    "מסנן התוכן של מודל התמונה דחה את התיאור. זה קורה בעיקר לדמויות, לוגואים ומותגים מוגנים בזכויות יוצרים — ניסיון נוסף באותו תיאור ייחסם גם הוא. נסחו את הבקשה במילים שלכם (למשל \"עכבר מצויר\" במקום שם הדמות) ונסו שוב.",
  ...
  design: {
    ...
    sizeOutOfRange: (value: number, lo: number, hi: number) =>
      `המידה ${value} מ״מ אינה אפשרית — הטווח הוא ${lo}–${hi} מ״מ.`,
