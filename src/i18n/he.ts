...
export const he = {
  ...
  design: {
     ...
     *
     * הנוסח הקודם — "התמונה משמשת כתיאור בלבד" — לא היה מדויק, והפער הזה הוא
     * תלונה שהגיעה: התמונה אינה משמשת כלום. היא לא נשלחת, וגם המשפט שמפנה
     * אליה יורד מהפרומפט (`buildPrompt`), כי הוא היה מפנה את המנוע לתמונת
     * הכיתוב. מי שסימן "סקיצה" וקיבל עיצוב שאינו דומה לה קרא כאן שהיא נלקחה
     * בחשבון. עדיף לומר את זה מראש מאשר להסביר אחר כך.
     */
    textOverridesImage:
      "שימו לב: כשיש כיתוב, התמונה שהעליתם אינה נשלחת למנוע — מקום הרפרנס שמור לכיתוב. " +
      "כדי שהתמונה תשפיע, אפשר להסיר את הכיתוב, או לתאר במילים בתיאור החופשי מה בתמונה חשוב לכם.",
