 ...
 *
 * ===== זו נקודת האכיפה היחידה על `auth_user_id` (0024) =====
 *
 * עד 16.8 היה FK ל-`auth.users` שדחה ערך שאינו משתמש קיים. הוא הוסר, כי
 * ‏`auth.users` אינה בסכימה `public` ולכן אינה בגיבוי — ובשחזור לפרויקט חדש
 * ה-FK נכשל על נתונים שנטענו זה עתה, כלומר הגיבוי לא היה בר-שחזור בפעולה
 * אחת (המספרים והשחזור: `supabase/migrations/0024_drop_auth_users_fk.sql`).
 *
 * מה שנשאר: **הפונקציה הזאת היא המקום היחיד שכותב את השדה**, והערך שהיא
 * כותבת מגיע מסשן מאומת. שני דברים נובעים מזה, ושניהם חשובים למי שנוגע כאן:
 *
 *   · אין לכתוב `auth_user_id` משום מקום אחר. אין רשת מתחת.
 *   · צעד 2 **דורס** את הערך הקיים בכוונה — זה מה שמרפא מזהה שהתייתם
 *     (משתמש שנמחק ב-Supabase, פרויקט ששוחזר) בלי שאיש נוגע במסד.
 *
 * מה שהיה תפקיד ה-FK ואיננו — לספר שקרה — עבר ל-`dangling_auth_links()`,
 * שנקראת בסטטוס התפעולי (`lib/opsStatus.ts`).
 */
