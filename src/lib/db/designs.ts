...
export interface VersionCandidate {
  ...
  /**
   * dialogue mode — איזה מבין כיווני העיצוב של שלב הטקסט המועמד הזה **הוא**
   * (`RunInputs.designStage.spec` → `designs[designIndex]`).
   *
   * **למה זה נשמר כאן ולא נגזר בקריאה.** `picked_index` מצביע על מקום במערך
   * הזה, אבל בין המפרט לבין הסדר הזה עומדות ארבע טרנספורמציות —
   * `pickClosestRatio` (זורק ומסדר), מיון לפי סטטוס ומתיחה, `orderByVariety`,
   * ומסגור שזורק מועמד שנפל. אף אחת מהן אינה הפיכה, ולכן זהות שלא נצמדה
   * בזמן אמת אינה ניתנת לשחזור אחר כך.
   *
   * חסר = לא היה מפרט (כל מסלול אחר), או שהמיפוי לא היה אמין ולכן לא נרשם —
   * ראה `designIndexesFor` ב-api/generate. **חסר עדיף על ניחוש**: מפרט של
   * הכיוון הלא נכון מצהיר בביטחון על פריט אחר.
   */
  designIndex?: number;
}
