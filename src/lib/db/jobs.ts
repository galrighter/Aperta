...
/**
 * סגירת job — עם תשובה על מי סגר אותו. **הדרך היחידה לסגור.**
 *
 * ההבדל מעדכון רגיל הוא התניה על `status = 'running'`, והיא קיימת בגלל
 * הלקוחה: היא מושכת כל שנייה וחצי, ולכן שני קוראים יכולים למצוא את אותה גרסה
 * בו-זמנית — סקר והתאוששות, או סקר והבקשה שפתחה את ההרצה. סגירה לא מותנית
 * הייתה נותנת לשניהם לכתוב `done` בלי הבדל ביניהם, וכל אחד היה שולח את מייל
 * "העיצוב מוכן" — שני מיילים על אותו עיצוב.
 *
 * עד 14.8 הבקשה עצמה סגרה בלי תנאי, בהנחה שהיא הקורא היחיד בזמן שההרצה רצה.
 * ההנחה נשברה כשהלקוחה התחילה לצפות בשורה **במקביל** לבקשה
 * (`lib/client/api.ts`): הגרסה נכתבת בסוף `ingestCutouts` והשורה עדיין
 * ב-`saving`, כך שסקר שנוחת בין השתיים סוגר אותה לפני שהבקשה הספיקה.
 *
 * כאן רק העדכון שהעביר את השורה מ-`running` נוגע בשורה, ולכן `true` חוזר
 * לקורא אחד בלבד. הוא הבעלים של הסגירה הזו, וההתראה נתלית בו.
 *
