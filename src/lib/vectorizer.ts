...
export async function ingestCutouts(opts: {
  ...
  const validation_report = {
    ...
    // ומה שהיה ידוע **לפני**: הגשרים שחתכנו בכיתוב, בקואורדינטות המסגרת שהגרסה
    // הזו יושבת בה. `bridges` מספר מה עשינו, וזה מספר מה התכוונו — וההבדל
    // קריטי למי שימשיך מהגרסה הזו. בלי זה כל מסלול שממשיך ממנה (שיתוף → אימוץ)
    // מאבד את הידיעה איפה הגשר של כל אות אמור לשבת ובאיזה כיוון, ומטפל בחלל של
    // אות כאי בעיטור: גשר לפי הקישור הקצר ביותר, שעל `ם` הוא אות אחרת.
    letterBridges: framed.letterBridges,
  };
