  ...
  // ראו docs/REMOVED_CONSTRAINTS.md. סומכים על המודל; נחזיר אם ניתקל בבעיה.
  {
    ...
    for (const cut of n.cutouts) {
      for (const poly of cut) {
        // אותו predicate בדיוק שהמנקה מריץ על "פתח שלם שקטן מדי"
        // (`isCuttableOpening`). שני מימושים של אותה שאלה נבדלו כאן בסף —
        // והתוצאה הייתה שהמנקה השאיר בשקט את מה שהבדיקה הזו פוסלת.
        //
        // **מה שהבדיקה הזו אינה רואה, ומי כן.** היא שואלת על הפוליגון כולו,
        // ולכן שערה ש**יוצאת מתוך** פתח רחב עוברת אותה — היא חלק מפוליגון
        // שהתשובה עליו "כן". ב-AP-0165 (14.8) זה הפיק בדיוק את השורה
        // "All cutouts ≥ 0.5mm" על גאומטריה מלאה חוטים.
        //
        // מי שסוגר את זה הוא המנקה (`uncuttableParts`), שמסיר עכשיו על-קבוצה
        // של מה שנפסל כאן — כך שאין גאומטריה שהוא משאיר והבדיקה הזו נופלת
        // עליה. והשער האחרון הוא V4, שמודד רוחב על החומר עצמו ותפס את מה
        // ששניהם פספסו. הבדיקה כאן נשארת predicate בכוונה: להחמיר אותה
        // לרוחב-מודע פירושו לפסול כל פינה מעוגלת בקטלוג על רעש של עיגול.
