...
describe("הרוחב שנמסר", () => {
  it("קנה מידה אחיד בתוך הטווח — נלקח כמו שהוא", () => {
    // AP-0064 (נמדד): הוזמן 12, נמסר 12.01 / 12.10 / 11.92. המודל צייר קרוב
  ...
  it("מחוץ לטווח — הרוחב הוא בדיוק מה שהוזמן, לא גבול הטווח", () => {
    // AP-0068 (נמדד): הוזמן 61.4×10, המודל צייר יחס 7.05 מול 6.14. קנה מידה
  ...
  it("`stretch` הוא היחס בין שני המקדמים, ולא מדידה מול הרוחב שהוזמן", () => {
    // במסלול הרגיל ה-vectorizer מקבל את הרוחב שהוזמן כ-`height_mm` ומחזיר
    // אותו ב-viewBox, ולכן שתי ההגדרות זהות — וזה מה שנבדק כאן, על שני
    // המקרים: קנה מידה אחיד ומתיחה מלאה.
    expect(frameCutoutsDims(dims(61.4, 10), drawn(61.4, 10)).stretch).toBeCloseTo(1, 6);
    expect(frameCutoutsDims(dims(61.4, 10), drawn(70.51, 10)).stretch).toBeCloseTo(0.871, 3);

    // מה שהשתנה הוא מסגרת שהמודל צייר ברוחב **אחר** מהמוזמן — המקרה של
    // `storyFrameDims`. שם קנה המידה אחיד לגמרי, וזה מה שמדווח.
    const out = frameCutoutsDims(dims(160.4, 28.23), drawn(102.27, 18));
    expect(out.widthMm).toBe(28.23);
    expect(out.stretch).toBeCloseTo(1, 3);
  });
  ...
  it("שתי גרסאות של אותה הזמנה מקבלות את אותו רוחב", () => {
    // AP-0062 (נמדד): אותה הזמנה נתנה 10.5 בגרסה אחת ו-9.5 באחרת — 10% הפרש,
