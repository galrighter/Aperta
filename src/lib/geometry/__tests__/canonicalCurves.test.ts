 ...
 *
 * `normalizeD` מעגל מספרים ואינו נוגע בפקודות, ואילו `rescaleCutoutsSvg` זורק
 * על כל מה שאינו M/L/Z. נתיב עם עקומה נשמר בשקט כגרסה, ואז מפיל את הקריאה
 * כולה ברגע שהגרסה חוזרת למסגור — עריכה, או אימוץ של שיתוף.
 *
 * הווקטורייזר שלנו פולט M/L/Z בלבד (`core/svg_builder.py`), ולכן זה מכסה SVG
 * שלא אנחנו ייצרנו: קלט ל-`/api/validate`, או גרסה שנשמרה לפני שהוא עשה זאת.
 */
