...
export const FAB = {
  ...
  /**
   * **תצוגה בלבד. אינו נאכף, וזו החלטה ולא פער.**
   *
   * השורה הזאת אמרה "תקרה קשיחה לערגול" ו-`validate.ts` מעולם לא בדק אותה —
   * סתירה שהאבחון סימן כממצא (docs/FULL_AUDIT_2026-08.md, פרק 3, ממצא 7).
   * ההכרעה (גל, 16.8): **שטח פתוח גבוה אינו שובר בערגול**, ולכן אין מה
   * לאכוף — מה שהיה שגוי הוא ההצהרה, לא ההתנהגות.
   *
   * הערך נשאר כי `maxOpenAreaWarnPct` מתאר את אותו מדד ומשמש באזהרה; מה
   * שהוסר הוא ההבטחה. מי שיחזור לכאן ויחשוב "יש כאן באג" — אין.
   */
  maxOpenAreaFailPct: 30,
  ...
  /**
   * תקרה **גלובלית** ליום, על כל המשתמשים יחד.
   *
   * המכסה שמעליה היא פר-פרופיל, וזו הגנה על משתמש בודד — לא על החשבון.
   * ‏500 נרשמים בשעה, כל אחד עם 50 ליום, הם עד 25,000 יצירות מותרות ביום;
   * ההגנה היחידה שעמדה מול זה הייתה hard limit ידני בחשבון OpenAI, כלומר
   * מקום שאיש לא רואה עד שהוא נחצה (docs/FULL_AUDIT_2026-08.md, פרק 7).
   *
   * **300 ולא 25,000, ולמה דווקא.** בקצב הנוכחי (192 גרסאות מאז 21.7) זה
   * פי כמה מכל יום שהיה, כלומר לא ייגע בלקוחה אמיתית. ובקצה השני הוא תוחם
   * את הנזק: ‎$0.006 להדמיה ⇒ ‎~$1.8 ליום בתקרה, ועד ‎~$90 גם אם **כל**
   * ההרצות יידרדרו ל-fallback היקר (פי 20–50). זה סכום שאפשר לספוג; 25,000
   * הרצות ב-fallback אינו.
   *
   * להעלות כשיהיה קצב אמיתי — עם המספר שמצדיק אותו, לא "ליתר ביטחון".
   */
  GLOBAL_DAILY_GENERATION_LIMIT: 300,
  /** מאיזה אחוז מהתקרה יוצאת התראה. ‏80% הוא עוד יום שלם של מרווח. */
  GLOBAL_LIMIT_WARN_PCT: 80,
} as const;
