...
export const FAB = {
  ...
  /**
   * המינימום לגשר של **אות** (הכרעת גל).
   *
   * `minBridgeCut` הוא המינימום לגשר במבנה — מתכת שנושאת את הפריט ועוברת
   * ערגול. הגשר שמחזיק את החלל הסגור של אות אינו כזה: הוא מחזיק אי בשטח של
   * מילימטרים ספורים, ולא נושא שום עומס. אכיפת 1.5 מ"מ עליו לא שמרה על כלום
   * וכן הרסה את האות — קונטור של `e` בגובה 6 מ"מ הוא 0.85 מ"מ.
   *
   * מתחת ל-0.75 לא יורדים. במקרה כזה הגשר נשאר על המינימום ובולט לתוך האות,
   * והלקוחה מתבקשת לאשר שהכיתוב נראה לה תקין — החלטה ויזואלית, שלה.
   *
   * מספר אחד ולא טבלה לפי עובי: הוא נקבע לפריט 1.5 מ"מ, שהוא כל מה שמיוצר
   * היום. מוצר בעובי אחר מחייב לחזור לכאן.
   */
  minLetterBridgeMm: 0.75,

  /**
   * הרוחב שמתחתיו צוואר **נחשב** צר מדי, כמרחק מ-`minLetterBridgeMm`.
   *
   * `minLetterBridgeMm` היא המידה שאליה **מעבים**; זו הסובלנות סביבה. ההפרש
   * אינו קוסמטי: ה-kerf הוא 0.12 מ"מ, סדר גודל מעל הסובלנות הזו, והמתאר שחוזר
   * מהמעקב נושא נקודה כל 0.02–0.1 מ"מ. צוואר של 0.72 מול רצפה של 0.75 אינו
   * "צוואר שיישבר" — הוא אותו צוואר, נמדד ברעש.
   *
   * **המספר הזה מחייב את שני הצדדים, וזה כל העניין.** הוא נולד ב-`thickenBridges`
   * (AP-0075) כדי שהתיקון לא ימרח פס מתכת על אות בשביל מאיות מ"מ — התיקון הוא
   * `linkRect` ברוחב מלא ולא "להוסיף את ההפרש". אבל הוא נוסף למתקן בלבד, בעוד
   * ש-V4 המשיכה להפיל על 0.75 חד. התוצאה הייתה **תחום מת**: צוואר ב-[0.70, 0.75)
   * נכשל בוודאות ואי אפשר היה לתקן אותו, כי המתקן ויתר עליו במכוון והשופט פסל
   * אותו במכוון. נמדד על עיצוב 99 — חמישה מקומות, "לא ניתן לייצור", ושום יצירה
   * חוזרת לא הייתה מוציאה אותו משם.
   *
   * מאז יש מספר אחד: `NECK_FLOOR_MM` הוא מה שנאכף, והמתקן מוותר בדיוק עליו.
   */
  letterBridgeToleranceMm: 0.05,
   ...
   * AP-0065 (נמדד) הוזמן להיקף 11 ס"מ, החישוב נתן 104.4 מ"מ, ונשמר 125.0.
   ...
   *
   * `lengthLimitMm` הוא דבר אחר לגמרי, ואסור לבלבל ביניהם: לא טווח מוצע אלא
   * **גבול השפיות** של הצינור. הוא לא חותך שום מידה — הוא דוחה אורך שאינו יכול
   * להיות פריסה של תכשיט על גוף, ומחזיר שגיאה במקום להריץ.
   *
   * נמדד ב-AP-0077 (3.8.26): הוזמן צמיד בהיקף 10 מ״מ (סנטימטרים בשדה שמבקש
   * מילימטרים), החישוב נתן פריסה של 15.8 מ״מ על רוחב 18, והפרומפט ביקש ממודל
   * התמונה "piece is 15.8mm long and 18mm wide … 0.9 times longer than it is
   * wide" — כלומר ריבוע. המודל צייר בדיוק את זה, הווקטורייזר עקב אחריו ב-IoU
   * 0.93, וההרצה נרשמה `approved`. שום שער בצינור לא היה יכול לתפוס את זה
   * **אחרי** הרנדר: לא המודל סטה ולא הפרומפט השתנה, הקלט היה בלתי אפשרי.
   * ולכן השער כאן, לפני שיוצאת קריאה שעולה כסף.
   *
   * הגבולות רחבים בכוונה — הם נגזרים מהקצה של מה שהממשק מרשה להזין ועוד
   * מרווח, ולא ממה שנפוץ. פרק יד של 9 ס״מ (תינוק) ב-snug נותן פריסה 73.6 מ״מ,
   * ולכן התחתית 70 ולא 80; פרק יד של 26 ס״מ ב-loose נותן 261, ולכן התקרה 280.
   */
  products: {
    ...
    bracelet: {
      ...
      lengthLimitMm: [70, 280] as [number, number],
    ...
    ring: {
      ...
      lengthLimitMm: [25, 130] as [number, number],
...
/**
 * הרצפה שצוואר **נאכף** מולה — נגזרת, ולא מספר שני.
 *
 * שני הצדדים חייבים לקרוא מכאן: `thickenBridges` מוותר על צוואר שרחב מזה,
 * ו-V4 מפילה צוואר שצר מזה. כל עוד זה אותו ערך, כל צוואר שהוולידציה פוסלת הוא
 * צוואר שהמתקן ניסה להרחיב — ואין מצב שנפסל בלי שניסינו. הפרדה בין השניים היא
 * בדיוק התחום המת שתועד ב-`letterBridgeToleranceMm`.
 *
 * העיגול אינו קוסמטי: 0.75 - 0.05 הוא 0.7000000000000001 בנקודה צפה, וזה גם
 * מה שהיה נכתב בהודעה שהלקוחה רואה.
 */
export const NECK_FLOOR_MM =
  Math.round((FAB.minLetterBridgeMm - FAB.letterBridgeToleranceMm) * 100) / 100;
