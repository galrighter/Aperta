...
describe("הטווח שמותר לבקש", () => {
  ...
  it("הרצפה היא של המוצר, ולא מספר אחד לשניהם", () => {
    // טבעת **צריכה** להיות צרה יותר מצמיד — מדידת הצר ביותר ב-81 המועמדים
    // השמורים: חציון 4.72 בטבעת מול 13.84 בצמיד.
    expect(MIN_WIDTH_MM).toEqual({ ring: 2, bracelet: 3.5 });
  });

  it("בצמיד הקצה הדק נחתך כבר באורך העוגן", () => {
    // 160.4/47.5 = 3.38 מ"מ, מתחת ל-3.5.
    const [wide, thin] = ratioBandFor("bracelet", 160.4);
    expect(wide).toBe(2.4);
    expect(thin).toBe(45.8);
  });

  it("בפריט קצר הוא נחתך עמוק יותר — היחס משתנה, לא הרוחב", () => {
    // פרק יד קטן: פריסה 132 מ"מ. 132/47.5 = 2.78 מ"מ, הרבה מתחת ל-3.5.
    const [wide, thin] = ratioBandFor("bracelet", 132);
    expect(wide).toBe(2.4);
    expect(thin).toBe(37.7);
    // וזו ההגדרה: בקצה החדש הרוחב הוא בדיוק המינימום, לא פחות.
    expect(132 / thin).toBeCloseTo(MIN_WIDTH_MM.bracelet, 1);
  });

  it("בטבעת הרצפה אינה נוגעת — גם בקטנה שבטבלה", () => {
    // מידה 1, פריסה ~39.8 מ"מ: 39.8/12.25 = 3.25 מ"מ, הרבה מעל 2.
