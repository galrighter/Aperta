 ...
 *
 * ⚠ **מה שנשאר לוודא.** פריט שמצויר לרוחב מלא של הקנבס מקבל ~1380 פיקסלים על
 * הציר הארוך — ‎8.6 px/mm על צמיד של 160 מ"מ, שבו פתח של 0.5 מ"מ הוא ארבעה
 * פיקסלים בלבד. אם התוצאה גסה, `high` היא הידית (השורה כאן).
 *
 * ⚠ **מה שאינו ידית — מספר השורות.** נכתב כאן קודם ששורה אחת "נותנת פי
 * שלושה פיקסלים לפריט". זה לא נכון: הציר הארוך של הפריט מוגבל ב**רוחב
 * הקנבס** (1536) ולא בגובה התא, ולכן ‎px/mm זהה ב-3×1 וב-1×1 — שורה אחת רק
 * מוסיפה לבן מעל ומתחת, במחיר של חלופה אחת במקום שלוש. הידית האמיתית השנייה
 * היא **כמה גדול הפריט מצויר בתוך הרוחב הזה**, וזו כבר דרישה קשיחה בפרומפט
 * (`designStage.ts`, פסקת SCALE) — ציור ב-60% מהרוחב מוריד ל-5.2 px/mm,
 * מתחת לרצפה של 6 שמתועדת ב-`render/canvas.ts`, ושם הפתחים כבר לא שורדים את
 * ה-conditioning. זה מה שקרה בעיצוב 165.
 *
 * העלות:
 *
 ...
 * הדומיננטי בחשבון של מודל תמונה. **ואי אפשר לאמת אותם מהיומן**: הקופסה אינה
 * שומרת את `usage` של תשובת מודל התמונה — `_extract` מושך משם את התמונה
 * בלבד. זה הפער שצריך לסגור לפני שמסתמכים על מספר שאינו השורה הראשונה.
 */
