...
import { LANDSCAPE, PORTRAIT, portraitEnabled, type Canvas } from "@/lib/render/canvas";
...
/**
 * מודל התמונה והמאמץ שהמסלול הזה רץ בהם — במקום אחד, כדי ששינוי יהיה שורה.
 *
 * **הזול משניהם, כי שלב העיצוב עושה את העבודה הקשה.** ‏`gpt-image-1` ו-`high`
 * שניהם נוסו כאן, ושניהם ירדו מאותו נימוק: מה שחסר לא היה כוח ציור אלא
 * פרשנות. מודל התמונה קיבל סיפור והתבקש לפרש אותו ולצייר בבת אחת, ואת הראשון
 * הוא עושה רע — מה שחזר היה מלבן שחור עם חיתוך רחב אחד, שלוש פעמים.
 *
 * מאז שלב הטקסט (lib/story/designStage.ts) הוא כבר לא מפרש: הוא מקבל הוראות
 * ציור גיאומטריות מפורשות ומבצע אותן. לזה `low` אמור להספיק — וזו בדיוק
 * הטענה שהניסוי הזה בודק.
 *
 * ⚠ **מה שנשאר לוודא.** בקנה המידה של שלוש שורות כל פריט מצויר בפס של בערך
 * 1380×170 פיקסלים — ‎8.6 px/mm על צמיד של 160 מ"מ, שבו פתח של 0.5 מ"מ הוא
 * ארבעה פיקסלים בלבד. אם התוצאה עדיין גסה, `high` היא הידית הראשונה למשוך
 * (השורה כאן), ומספר השורות היא השנייה — שורה אחת נותנת פי שלושה פיקסלים
 * לפריט, במחיר של חלופה אחת במקום שלוש.
 *
 * העלות:
 *
 *     gpt-image-1-mini · low   · 1536x1024  = $0.0057    ← כאן, נמדד (31/07)
 *     gpt-image-1-mini · high  · 1536x1024  ≈ $0.05–0.09 ← נוסה, ירד. לא נמדד
 *     gpt-image-1      · high  · 1536x1024  ≈ $0.25      ← נוסה, ירד. לא נמדד
 *
 * ועוד ~$0.002 לשלב הטקסט, כלומר ~$0.008 להרצה שמחזירה שלושה עיצובים.
 *
 * ⚠ **רק השורה הראשונה נמדדה** — מה-`usage` שהתשובה החזירה על בדיוק אותה
 * בקשה (ראה imagegen.ts). האחרות הן מספרים שנישאו מדף התמחור ולא אומתו מול
 * הרצה; הטווח נגזר מטוקני הפלט (~408 ב-low מול ~6,240 ב-high), שהם החלק
 * הדומיננטי בחשבון של מודל תמונה.
 *
 * שלוש השורות האלה נמדדו ידנית, פעם אחת. מכאן והלאה אין צורך: כל הרצה שומרת
 * את ה-`usage` שלה ביומן (`RunInputs.renderUsage`), ליד המודל והמאמץ שקבעו
 * אותו. הטבלה כאן היא נקודת ההתחלה — היומן הוא מה שאומר מה זה עלה באמת.
 */
...
/**
 * מה שרץ כששלב הטקסט נפל בכל הניסיונות.
 *
 * **זה לא "מודל גיבוי" אלא פיצוי על עבודה שלא נעשתה.** הבחירה ב-`mini` וב-`low`
 * למעלה נשענת כולה על כך ששלב הטקסט כבר תרגם את הסיפור להוראות ציור
 * גיאומטריות — למודל התמונה נשאר לבצע. כשהשלב הזה נפל, הפרומפט חוזר להיות
 * זה של שלב אחד, ומודל התמונה מתבקש שוב **גם לפרש וגם לצייר**. את הראשון
 * `mini` עושה רע, וזה מה שהניסוי המקורי הראה: מלבן שחור עם חיתוך רחב אחד,
 * שלוש פעמים. לכן דווקא כאן צריך את המודל החזק ואת המאמץ הגבוה.
 *
 * ⚠ **זה עולה פי 20–50** (‏$0.0057 מול סדר גודל של $0.1–$0.3 — לא נמדד; זו
 * ההזדמנות הראשונה למדוד, כי `renderUsage` נשמר). זו הסיבה שהמעבר לכאן מעיר
 * את התורן ושולח התראה בטלגרם ואינו רק נופל בשקט: מסלול שרץ ככה שבוע בלי
 * שאיש ידע הוא חשבון גדול על עיצובים פחות טובים.
 *
 * הלקוחה **אינה** מקבלת התראה (החלטת גל, 14.8): היא מקבלת תוצאה, ואינה אמורה
 * להרגיש שמשהו קרה.
 */
export const STORY_RENDER_FALLBACK = {
  model: "gpt-image-2",
  quality: "high",
} as const;
...
/**
 * הקנבס של המסלול — **לאורך, קבוע** (החלטת גל), פרט לכיבוי החירום.
 *
 * `PORTRAIT_CANVAS=0` ממשיך לחלוש גם כאן. הוא לא הועתק לכאן כהעדפה אלא כחוזה
 * תפעולי: הוא נכתב כדי שאפשר יהיה לחזור מקנבס לאורך בלי revert ובלי פריסה
 * מחדש — למשל אם הקופסה תפסיק להבין `size` — וצרכן חדש של לאורך שמתעלם ממנו
 * הופך את הידית הזו לחצי ידית, בדיוק ברגע שמושכים אותה.
 *
 * **למה קבוע ולא `canvasFor`.** במסלול הרגיל צורת הקנבס נגזרת מהאורך שהוזמן,
 * כי האורך ידוע לפני שהמודל רץ. כאן הוא **לא**: הלקוחה מספרת, מקבלת, ורק אז
 * נמדדת (§4.1 ב-STORY_FLOW_PLAN) — הרשומה מחזיקה באותו רגע אורך ברירת מחדל
 * בלבד. כלומר `canvasFor` היה בוחר כאן לפי מספר שהוא עוגן תכנוני ולא מידה,
 * וקובע את צורת התמונה מנתון שאיש לא בחר.
 *
 * מכאן שגם היצירה וגם בקשת השינוי עוברות בקבוע הזה. עד עכשיו רק הראשונה
 * הייתה מקובעת (ללרוחב), והשנייה התגלגלה ל-`canvasFor` — כך שאותו עיצוב יכול
 * היה להיווצר על צורה אחת ולהיערך על שנייה, מול תמונת ייחוס שמוסגרה מחדש.
 *
 * ⚠ **המחיר, במפורש.** הציר הארוך של הפריט מקבל 1024 פיקסלים במקום 1536
 * (~33% פחות), ושלוש שורות בקנבס לאורך מושכות את המודל ליחס טבעי של 6.97
 * במקום 9.05 — כלומר פריטים קצרים ורחבים יותר ממה שיצא עד עכשיו. במסלול הזה
 * הרוחב ממילא נגזר ממה שצויר (`storyFrameDims`), ולכן זה משנה את **הצורה**
 * ולא מותח אותה. מה ששומר על הייצור הוא הוולידציה, כמו תמיד.
 *
 * ⚠ **ומה שנשאר לוודא, במספר.** הפריט נפרס על הציר הארוך, שהוא כאן ‎1024·0.9 =
 * ‎921.6 פיקסלים במקום ‎1382.4. פתח חייב ~3 פיקסלים כדי לשרוד את המעקב, כלומר
 * הפתח הדק ביותר שעובר הוא ‎1/307 מאורך הפריט — ‎0.52 מ"מ על צמיד של 160 מ"מ,
 * מול ‎0.35 מ"מ לרוחב. המינימום לייצור הוא ‎0.5 מ"מ, כלומר אנחנו יושבים בדיוק
 * על הגבול. הכשל הצפוי אינו פריט פגום אלא **מועמד שנפסל** — הוולידציה רצה על
 * הגאומטריה שאחרי המסגור — ולכן מה שנמדד הוא שיעור הפסילה ולא איכות החיתוך.
 *
 * שלוש המדידות שיגידו אם זה עבד יושבות כבר ביומן על כל הרצה, ואין צורך
 * במכשור נוסף: `drawnRatio` (הצורה שיצאה), `approvedPanels`/`panelVerdicts`
 * (כמה מתוך שלושה שרדו, ועל מה נפלו), ו-`metrics.iou` (כמה נאמן המעקב היה).
 * `canvasSize` נשמר על כל שורה מאז שהקנבס הפך לפרמטר, ולכן ההעמדה מול הרצות
 * "לפני" היא סינון ביומן ולא ניסוי שצריך להריץ.
 */
export const storyCanvas = (enabled = portraitEnabled()): Canvas =>
