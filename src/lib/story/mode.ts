 ...
 *
 * **המודל הקטן, באיכות גבוהה.** התמונה מחולקת לשלוש שורות, ולכן כל פריט מצויר
 * בפס של בערך 1380×170 פיקסלים — ‎8.6 px/mm על צמיד של 160 מ"מ, שבו פתח של
 * 0.5 מ"מ הוא ארבעה פיקסלים. ב-`quality: "low"` מה שחזר משם היה גושים: מלבן
 * שחור עם חיתוך רחב אחד, שלוש פעמים, בלי שהצללית החיצונית תשתתף. `high` היא
 * מה שקונה את הדיוק בקנה המידה הזה.
 *
 * **מה שהמודל הגדול היה קונה, שלב העיצוב קונה במקומו.** `gpt-image-1` נוסה
 * כאן וירד: מה שחסר לא היה כוח ציור אלא פרשנות, ואת זה עושה עכשיו שלב הטקסט
 * (lib/story/designStage.ts) שרץ לפניו. מודל התמונה מקבל הנחיות גיאומטריות
 * מפורשות ולא סיפור — הוא לא צריך לחשוב יצירתית, הוא צריך לצייר.
 *
 * העלות:
 *
 *     gpt-image-1-mini · low   · 1536x1024  = $0.0057   ← נמדד (31/07)
 *     gpt-image-1-mini · high  · 1536x1024  ≈ $0.05–0.09 ← כאן, **לא נמדד**
 *     gpt-image-1      · high  · 1536x1024  ≈ $0.25      ← לא נמדד
 *
 * ועוד ~$0.002 לשלב הטקסט. קריאה אחת מכל אחד להרצה.
 *
 * ⚠ **רק השורה הראשונה נמדדה** — מה-`usage` שהתשובה החזירה על בדיוק אותה
 * בקשה (ראה imagegen.ts). שתי האחרות הן מספרים שנישאו מדף התמחור ולא אומתו
 * מול הרצה. הטווח בשורה השנייה נגזר מטוקני הפלט: 1536x1024 הוא ~408 טוקנים
 * ב-low ו-~6,240 ב-high, כלומר ×15 על החלק שדומיננטי בחשבון של מודל תמונה.
 *
 * **ואי אפשר לאמת את זה מהיומן**, כי הקופסה אינה שומרת את `usage` של תשובת
 * מודל התמונה — `_extract` מושך משם את התמונה בלבד. זה הפער שצריך לסגור לפני
 * שמסתמכים על מספר כאן.
 */
