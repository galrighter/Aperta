...
export async function runRenderJob(input: RenderJobInput): Promise<RenderJob> {
  ...
  let resp: Response;
  try {
    resp = await fetch(`${serviceUrl()}/api/generate`, {
      method: "POST",
      headers,
      body: JSON.stringify({
        prompt: input.prompt,
  ...
  try {
    ...
    throw new ApiError(
      "render_bad_response",
      `Render service returned non-JSON (${resp.status}): ${text.slice(0, 300)}`,
      502,
    );
  }
  if (!resp.ok) {
    ...
    const code = detail?.error_code === "RENDER_FAILED" ? "llm_error" : "render_failed";
