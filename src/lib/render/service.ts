...
export interface RenderJobInput {
  ...
  quality?: string;
  /**
   * מה שמזהה את ההרצה מול הקופסה, במקום גוף הבקשה עצמו.
   *
   * **למה זה נדרש.** ברירת המחדל גוזרת את מזהה ה-job מ-`JSON.stringify(request)`,
   * ובזה נשענת ההבטחה שניסיון חוזר אחרי ניתוק אוסף רנדר ששולם במקום לקנות
   * שני. ההבטחה הזו מתקיימת כל עוד אותו קלט מייצר אותה בקשה — וזה בדיוק מה
   * שנשבר במסלול Story הדו-שלבי: הפרומפט נושא בתוכו את ה-JSON של שלב העיצוב,
   * שנוצר מחדש בכל ניסיון ולעולם אינו זהה. בלי המפתח הזה כל ניתוק היה קונה
   * רנדר שני.
   *
   * מה שנמסר כאן הוא **הקלט היציב** של אותה הרצה — הפרומפט של שלב העיצוב,
   * המידות והתכנון — ולכן ניסיון חוזר מגיע לאותו job. הקופסה מחזירה אז את
   * התוצאה השמורה ומתעלמת מהפרומפט השני, וזו התנהגות נכונה: הרנדר כבר נעשה.
   *
   * ריק = ההתנהגות הקיימת, כלומר גוף הבקשה.
   */
  requestKey?: string;
...
export async function runRenderJob(input: RenderJobInput): Promise<RenderJob> {
  ...
  const boxJobId = input.jobId
    ? await deriveRenderJobId(input.jobId, input.requestKey ?? JSON.stringify(request))
    : null;
