 ...
 *
 * **הטבלה היא גם הטווח.** `idMmFromUsSize` צובט לקצוות שלה, `sizeIssue` דוחה
 * מה שמחוצה לה, ומדריך המידות מדפיס אותה כמות שהיא — כלומר מה שלא כתוב כאן
 * פשוט לא ניתן להזמנה. לכן היא נפרסת על כל טווח הענידה ולא על הנפוץ בלבד.
 *
 * **4–13 היה הטווח עד 3.8.26**, ומקורו בטבלה של ביקורת המידות ולא בגבול ייצור.
 * הוא חתך משני הקצוות אנשים אמיתיים: מידות ילדים (1–3.5) מצד אחד, ואצבע גדולה
 * של גבר (13.5–16) מצד שני — ושניהם קיבלו בשקט את מידת הקצה, כי הצביטה
 * ב-`idMmFromUsSize` לא אמרה דבר. 1 ו-16 הם קצות הטבלאות המסחריות: מתחת ל-1
 * מדובר באצבע של תינוק, ומעל 16 בהזמנה מיוחדת.
 *
 * שתים־עשרה השורות שנוספו נגזרו מהשיפוע של הטבלה עצמה
 * (`MM_ID_PER_US_SIZE` = 0.83 מ"מ למידה, מעוגן ב-4 → 14.86), ולא מטבלה אחרת:
 * שילוב של שני מקורות היה יוצר מדרגה בדיוק בתפר, ומדרגה בסולם מידות היא
 * חצי מידה שמישהו יקבל בטעות. הסטייה מהצ'ארטים המפורסמים בקצה העליון היא
 * ‎~0.2 מ"מ, כרבע מידה — קטנה מהסבולת של טבעת פתוחה (ראו `ringDisclaimer`).
 */
export const US_RING_ID_MM: Record<string, number> = {
  "1": 12.37, "1.5": 12.79, "2": 13.20, "2.5": 13.62, "3": 14.03, "3.5": 14.45,
  ...
  "12.5": 21.89, "13": 22.33, "13.5": 22.75, "14": 23.16, "14.5": 23.58,
  "15": 23.99, "15.5": 24.41, "16": 24.82,
};
