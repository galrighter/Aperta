 ...
 * תקרה לסבב.
 *
 * הקנרית מייצרת 12 שורות ביום, והסבב רץ כל שעתיים — כלומר בשגרה יש כאן שורה
 * אחת, לכל היותר שתיים. התקרה קיימת בשביל הסבב **הראשון**, שפוגש היסטוריה
 * שלמה, ובשביל המקרה שבו המתזמן שתק ליום־יומיים: 200 שורות הן ~1,000 קבצים
 * למחיקה, מה שעדיין נכנס בנוחות בבקשה אחת, ומה שמעבר לזה ייאסף בסבב הבא.
 */
