 ...
 *
 * ⚠ **ולמה הוא לא הפרומפט הראשי, גם אם יתברר שהוא מונע חיתוך** (החלטת גל):
 * **צמצום עולה פיקסלים, והם מועילים בעיצוב עם פרטים קטנים.** על צמיד באורך
 * ברירת המחדל, `COL_FILL` (0.9) נותן 8.62 px/mm ולכן הפתח המינימלי (0.5 מ"מ)
 * מצויר ב-4.3 פיקסלים; ב-80% זה יורד ל-7.66 px/mm ו-3.8 פיקסלים, מול רצפת
 * הישרדות של 3 (`MIN_FEATURE_PX` ב-render/panels.ts). זה עדיין מעל הרצפה, אבל
 * המרווח נחתך בחצי — וזה בדיוק העיצוב הצפוף שמשלם, זה שיש בו מה לאבד.
 *
 * מכאן החלוקה: ברירת המחדל שומרת על הרזולוציה, והמסגור נכנס רק אחרי שראינו
 * מתכת על הגבול. שם החלופה אינה "פחות פיקסלים" אלא "פריט קטוע", ואז זו עסקה
 * טובה. **החלפה גורפת הייתה משלמת את המחיר הזה על כל הרצה כדי להגן על ~4%.**
 */
