...
- **`0004_accounts.sql` לא רצה** — רישום המשתמשים והמספר הסידורי כתובים, ושניהם
  דורשים את העמודות שהיא מוסיפה. **להריץ לפני ה-merge**, לא אחריו.
...
- **מסלול הזמנה וחיוב** — מיגרציה, `ADMIN_TOKEN`, התראה במייל, סליקה, חשבונית.
