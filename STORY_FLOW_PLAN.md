...
### 4.5 הרוחב שנגזר חוזר אל המצב
ההזמנה שולחת `widthMm` מה-viewBox של הגרסה אבל `displayedTotal` מ-`priceOf`,
שקורא את המצב — והשרת פוסל ב-409 (`price_changed`) כששני המספרים אינם מסכימים.
במסלול הרגיל הם כמעט תמיד זהים; במסלול Story הרוחב נגזר, ולכן המצב מתעדכן ממנו
ברגע שיש גרסה. בלי זה כל הזמנה במסלול Story הייתה נדחית בקופה.

מדיניות התמחור עצמה **לא נגעתי בה**: התוספת לפי מ"מ ממשיכה לחול כמו בכל הזמנה,
והיא מוצגת בסיכום לפני התשלום. אם יוחלט שבמסלול Story הרוחב לא אמור להשפיע על
המחיר — זהו תנאי אחד ב-`priceOf`.
