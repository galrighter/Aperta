...
/docs
  ...
  sizing-fit-review.md      # ביקורת הנדסית על מחקר המידות/ההתאמה מול מה שהמערכת עושה בפועל.
                            # מקור האמת למודל המידות (K-factor, Fit, פער, Springback, מנדרלים).
  research/sizing-ux-research.md  # מחקר המידות ו-UX כפי שהתקבל — verbatim, ללא שינוי.
```
...
8. **אין ישות "מידה" במערכת** — המשתמש מזין `length_mm` (אורך פריסה לחיתוך) ולא מידת טבעת או היקף פרק יד. חסרים: מודל מידות מבוסס ציר ניטרלי, רמות Fit לצמיד, ופיצוי רוחב. הניתוח המלא, כולל טבלאות אורך פריסה מוכנות וסדר תשואה, ב-`docs/sizing-fit-review.md`. שם גם מתועדים שני באגי מידה קיימים: ברירת המחדל של טבעת (54 מ"מ → יוצא US 6.4 במקום 7), ורדיוס ההדמיה ב-`Preview3D` (ID גדול ב-2·K·t = 1.5 מידות טבעת).

9. **`gap_mm` — מיתר או קשת?** לא מוגדר בשום מקום, ושני מקומות בקוד מניחים הנחות שונות. ההכרעה המומלצת (מיתר, כי זה מה שקליבר מודד) והמשמעות שלה ב-`docs/sizing-fit-review.md` §2.2.
