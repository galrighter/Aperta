...
> | # | נושא | חומרה | הערה |
> |---|------|-------|------|
> | O4 | `npm audit` — 6 פגיעויות | 🟢 נמוך | 5 high + 1 moderate, **כולן ב-`sharp`/libvips דרך wrangler** — כלי פיתוח, לא פרודקשן. תיקון דורש downgrade major של next |
...
> | O6 | פגמי קופי ותצוגה | 🟢 נמוך | §5 (הסעיף האחרון): "אל תנסה שוב עכשיו" ליד כפתור "נסה שוב", טקסט שגיאת JS גולמי ב-UI, אי-עקביות מגדר |
> | O7 | `geometry-service` נקשר ל-`0.0.0.0` בניגוד להערה שלו | 🟢 קוסמטי | האימות שם כבר fail-closed והוא בתוך קונטיינר מאחורי nginx — מה שנשאר הוא ההערה |
>
> #### שלוש שורות שהוסרו מכאן — הן כבר היו סגורות כשנכתבו
>
> | היה רשום | בפועל |
> |---|---|
> | O1 — כליאת פוקוס במודלים | `src/lib/client/useDialog.ts` עושה מיקוד־פנימה, כליאה, החזרה ל-opener ו-Escape; שלושת הדיאלוגים משתמשים בו |
> | O2 — `restoreBridges` בלי windowing | **נבדק ונדחה.** החלון נמדד ויצא פי 3 **גרוע** יותר (10,376 קודקודים: 1,874ms → 6,063ms), כי ה-`intersection` פר-אי יקר מהסריקה שהוא חוסך. הנימוק בקוד, ליד הקריאה |
> | O3 — fan-out בלתי מוגבל | `PANELS_PER_RENDER_CAP` ב-`vectorizer/app/generate.py` חוסם לשלושה פאנלים לכל תא ברשת, ומדווח על מה שנחתך |
>
...
> לממש הצעה שכבר נמדדה ונדחתה. **שורה בטבלה הזו עולה grep אחד לפני שכותבים
> אותה.** מסומן: אומת מול הקוד ב-4.8.2026.
>
