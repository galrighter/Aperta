...
> | מה | מה קרה | תיקון |
> |----|--------|-------|
> | isolate שנהרג אחרי שמירת הגרסה | RM-0084: עיצוב נשמר, שורת ה-job נתקעה ב-`saving`, והלקוחה קיבלה "היצירה נכשלה" | `lib/jobRecovery` — הסקר שואל אם ההרצה הפיקה גרסה לפני שהוא מכריז על כישלון |
