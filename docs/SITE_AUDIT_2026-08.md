...
>
> **שתי טבלאות נושאות את המצב:** *פתוח* — מה שעוד לא נגמר, ולידה *נדחה* — מה
> שנשקל, נמדד, והוכרע שלא ייעשה. השנייה קיימת כי הצעה שנדחתה על מדידה ונעלמת
> מהמסמך מוצעת שוב — וזה קרה בפועל ל-windowing של `restoreBridges`. *נסגר*
> למטה הוא הארכיון.
>
> ### פתוח
>
> **שניים, ואף אחד לא חוסם.** O4 (כלי פיתוח) ו-O8 (שכבת הנתונים, שנפתחה ב-6.8
> ועוד לא הושלמה). O5/O6/O7 נסגרו — ראו *נסגר* למטה.
>
> | # | נושא | חומרה | הערה |
> |---|------|-------|------|
> | O4 | `npm audit` — **פגיעות אחת** (היו 8) | 🟢 נמוך | `next@16` נעשה (5.8) וסגר את `next`+`postcss`+`sharp`; `undici` נסגר ב-`overrides` (יש בדיקה חודשית שתגיד מתי אפשר להסיר). מה שנשאר הוא `fast-xml-parser` בלבד, וההתראה שלו היא על `XMLBuilder` — אנחנו מייבאים `XMLParser` בלבד (`geometry/normalize.ts:1`) ולא בונים XML איתו. **לא נגיש בקוד שלנו** |
> | O8 | שכבת נתונים — **5 מתוך 9 הומרו** | 🟢 נמוך | נפתח 6.8. `useAdminData`/`useAdminPages` מחזיקים את סולם התגובות פעם אחת במקום שבעה עותקים. מה שנשאר מפורט ב-*שכבת הנתונים* למטה — שום דבר שבור, וההשתקות שנותרו נושאות נימוק |
>
> ### `next@16` — נבדק ב-4.8, בוצע ב-5.8
>
> הבדיקה המקורית נשארת כאן כי היא הפריכה שתי עובדות שהיו רשומות ב-O4. השדרוג
> עצמו בוצע אחריה, וחשף דבר אחד שהבדיקה לא יכלה לדעת — ראו **`middleware` מול
> `proxy`** למטה.
>
> | מה נבדק | התוצאה |
> |---|---|
...
> | `npm run lint` תחת 16 | **נשבר.** `FlatCompat` מתפוצץ (`Converting circular structure to JSON`) כי `eslint-config-next@16` כבר פלאט. המיגרציה היא 4 שורות ב-`eslint.config.mjs`, ואחריה צצים 20 שגיאות מכללי React Compiler. **שולם מראש על `next@15` (5.8)** — ראו למטה |
...
>
> ### שולם מראש — חוב ה-lint של `next@16` (5.8)
>
> הכללים של `eslint-config-next@16` קיימים עצמאית ב-`eslint-plugin-react-hooks@7`,
> ולכן נאכפו על `next@15` לפני השדרוג. `FlatCompat` פותר שמות תוספים מהשורש,
> כך שהתקנת v7 בשורש מספיקה — אין שורת קונפיג שמפעילה אותם במפורש.
>
> **תשע מעשרים תוקנו, ושלוש מהן היו באגים אמיתיים שאיש לא דיווח עליהם:**
>
> | מה | הבאג שהתגלה |
> |---|---|
> | `RolledStage` + `SharedPiece` | רמז הגרירה נקרא `matchMedia` פעם אחת ולא נרשם לשינויים — מכשיר היברידי (טאבלט עם מקלדת, מסך מגע בלפטופ) המשיך לראות את הרמז של סוג המצביע שהיה בהרכבה. אוחדו ל-`useCoarsePointer` עם `useSyncExternalStore` |
> | `ProgressBar` | האיפוס היה באפקט, כלומר **אחרי** הצביעה: הרצה חדשה נפתחה לפריים אחד על ההתקדמות של הקודמת ואז קפצה אחורה. עבר לגוף הרנדר |
> | `AccountGate` | אותו דפוס, ותוצאה גרועה יותר: המייל של המשתמש הקודם הבזיק בכל פתיחה של הטופס — בדיוק מה שהאיפוס נועד להסתיר |
>
> ועוד: `useDialog` ו-`Preview3D` כתבו ל-ref בגוף הרנדר (עבר לאפקט), `SiteHeader`
> סגר את התפריט אחרי הצביעה של העמוד החדש (הבהוב בניווט אחורה), ו-`ShareButton`
> קבע את זמינות גיליון השיתוף ברנדר שני מיותר.
>
> **אחת-עשרה נותרו, וכולן מאותו סוג — ולא ניתנות לתיקון מקומי.** הכלל מסמן את
> **הקריאה** לפונקציה שכותבת state, לא כתיבה סינכרונית: `OrderDetail.load`
> ו-`AdminGate.check` פותחות ב-`await` ומסומנות בדיוק כמו השאר. מה שהוא מבקש
> הוא לא לטעון נתונים מתוך אפקט — כלומר שכבת נתונים (SWR/React Query או טעינה
> בצד השרת), החלטה ארכיטקטונית ולא ניקוי lint. עד שתתקבל, כל אתר נושא
> `eslint-disable-next-line` עם הסיבה שלו, כדי שהכלל יישאר דלוק על קוד חדש.
>
> **מה שנשאר לשדרוג עצמו:** החלפת הגרסה, ו-`middleware`→`proxy`.
>
> ### `middleware` מול `proxy` — למה הקובץ **לא** הועבר (5.8)
>
> `next@16` מדפיס בכל בנייה שהקונבנציה `middleware` הוצאה משימוש, ומציע
> `npx @next/codemod middleware-to-proxy`. **ההצעה הזו שוברת את הפריסה.** נבדק,
> לא נוחש:
>
> | מה נוסה | מה קרה |
> |---|---|
> | העברה ל-`proxy.ts` | הבנייה עוברת, אבל `cf:build` נופל: «Node.js middleware is not currently supported» — כלומר **לא נוצר Worker בכלל** |
> | `export const runtime = "edge"` ב-`proxy.ts` | נדחה ע"י Next עצמו: «Route segment config is not allowed in Proxy file. Proxy always runs on Node.js runtime» |
> | להשאיר `middleware.ts` על 16 | **עובד.** נבנה כ-Edge (אומת מול `middleware-manifest.json`), ו-`cf:build` מייצר Worker |
>
> כלומר ב-16 `proxy` הוא Node בלבד, ו-`@opennextjs/cloudflare` (עד 1.20.2) אינו
> תומך בו. `middleware.ts` נשאר Edge ולכן נשאר. האזהרה בבנייה היא deprecation,
> לא תקלה — והנימוק יושב בראש `src/middleware.ts` כדי שלא יריצו שם codemod.
>
> **התוצאה החשובה:** השדרוג לא נגע בקובץ שמפנה `www` ל-apex. זה היה הסיכון
> שבגללו המלצנו לפצל את העבודה, והוא לא מומש.
>
> ### שכבת הנתונים — מה נעשה, ומה נשאר (6.8)
>
> שבעה מסכי בק־אופיס החזיקו כל אחד עותק של אותו טוען. `useAdminData` (יחיד)
> ו-`useAdminPages` (מעומד) מחזיקים את זה במקום אחד, על SWR.
>
> **המספר "11 השתקות" שהופיע כאן קודם היה שגוי** — שתיים מהן אינן טעינת נתונים
> כלל. תשע היו רלוונטיות.
>
> | מסך | מצב |
> |---|---|
> | `AdminGate`, `AdminOrders`, `AdminInquiries`, `OrderDetail`, `AdminDesigns` | **הומרו** |
> | `RunsLog` (×2) | **ממתין — מעבר בפני עצמו.** ראו למטה |
> | `DesignReadyWatch` | **ממתין — אחרון בכוונה.** ראו למטה |
> | `debug/page` | **לא יומר.** אינו משיכת משאב לפי כתובת אלא חישוב תוכנית דרך POST על מצב הטופס, קשור ל-`dirty`/`keepPrompt`. המרה שם עיוות |
> | `Preview3D` (×2) | **לא יומר לעולם.** כשל בבניית WebGL/גאומטריה, שמתגלה רק בניסיון |
>
> **מה נסגר חוץ מהכפילות:** מרוץ בין תשובות (לחיצה מהירה על שני מסננים — התשובה
> שחוזרת שנייה גוברת; רק `RunsLog` הגן על זה ביד), רענון בחזרה ללשונית, ורשימה
> ופירוט שחלוקים על אותה הזמנה אחרי שינוי סטטוס.
>
> #### `RunsLog` — למה הוא מעבר בפני עצמו
>
> הוא מחזיק שלושה דברים שאף מסך אחר לא מחזיק, ואחד מהם הוא תיקון מתועד שאסור
> לאבד:
>
> 1. **הפרדה בין כשל בעמוד הראשון לכשל ב"עוד".** ההודעה חייבת לשבת ליד הכפתור
>    שנלחץ; קודם היא ישבה מעל הרשימה, כלומר עשרים שורות מחוץ למסך, וכשל בהמשך
>    נראה כמו כפתור שלא עושה כלום. ב-SWR יש שגיאה אחת לכל סדרת העמודים, ולכן
>    ההפרדה צריכה להיגזר (יש עמודים טעונים = כשל ב"עוד").
> 2. **דה-דופליקציה לפי מזהה** — הרצה שנכתבה בין עמוד לעמוד יכולה לחזור פעמיים.
> 3. **סמן אטום** שמגיע מהעמוד הקודם (`lib/runs/cursor`).
>
> #### `DesignReadyWatch` — למה אחרון
>
> הוא **היחיד מהתשעה שרואה לקוחות**: החלון שאומר "העיצוב מוכן". הוא סוקר כל 6
> שניות עם `setInterval` + מאזין `visibilitychange` — בדיוק מה ש-`refreshInterval`
> ו-`revalidateOnFocus` עושים מובנה, ולכן זו גם ההמרה עם הרווח הגדול ביותר.
> ההמלצה: אחרי שהתבנית רצה בייצור על המסכים הפנימיים.
>
> #### מלכודת שנמצאה בדרך, ותוקנה לפני שהזיקה
>
> ההוק סינן אובדן הרשאה **תמיד**, גם כשהקורא לא מסר `onAuthLost`. `RunsLog` הוא
> בדיוק המקרה — הוא מאחורי שער משלו ואינו מוסר מטפל — ולכן 401 שם היה נעלם
> לגמרי: לא מטופל וגם לא מוצג. הסינון מותנה עכשיו בקיום מטפל. נמצא לפני ההמרה.
>
> #### מה שלא מכוסה
>
> אין DOM תחת vitest, ולכן הטסטים קוראים את המקור (כמו שאר הפרויקט). המסכים
> עצמם לא נבדקו בדפדפן — כולם פנימיים, וזו הסיבה שההמרה התחילה מהם.
>
> ### נדחה — ולמה
>
> **הטבלה הזו קיימת כדי שהצעה לא תיבדק פעמיים.** דחייה על מדידה היא תוצאה, לא
> היעדר טיפול; הצעה שנמדדה, נדחתה, ואז נעלמה מהמסמך חוזרת בסבב הבא כאילו איש
> לא נגע בה. שורה כאן נמחקת רק אם מדידה חדשה סותרת את הישנה.
>
> | # | ההצעה | ההכרעה | המדידה, והיכן היא בקוד |
> |---|-------|--------|------------------------|
> | O2 | windowing ל-`restoreBridges` — לחתוך את החיפוש לחלון סביב האי, כמו `WINDOW_MM` ב-`thickenBridges`, במקום סריקה O(V·E) | **נדחה על מדידה** (4.8) | החלון יצא **גרוע פי שלושה**: 10,376 קודקודים — 1,874ms מלא מול 6,063ms עם חלון; 3,464 קודקודים עם 12 איים — 233ms מול 257ms. הסיבה מבנית ולא סקאלרית: החלון דורש `intersection` פר-אי, וב-`restoreBridges` הלולאה היא כל אי מול הגוף הראשי בלבד (ב-`thickenBridges` היא כל גוף מול כל גוף), כלומר פחות סריקות לחלוק עליהן את מחיר החיתוך. עיצוב אמיתי נמדד ב-720–787 קודקודים. הנימוק המלא ליד הקריאה: `src/lib/geometry/restoreBridges.ts:274-282` |
>
> ### נסגר
>
> | ממצא | איפה |
> |------|------|
> | **מייל "העיצוב מוכן" לא נשלח במסלול ההתאוששות** (לא היה בדוח) | `api/generate/[jobId]` שולח אותו אחרי שהוא סוגר את ה-job. הסגירה מותנית ב-`running` (`claimJobDone`), ולכן רק סקר אחד שולח; העדות ל"גרסה ראשונה" היא `version_no` ולא `current_version_id`, שכבר מצביע על הגרסה בשלב הזה |
> | O5 — subpath בכיוון ליפוף הפוך | `pathToPolygons` מכריע חור לפי **הכלה** ולא לפי סימן בלבד: טבעת הפוכה שאינה בתוך חומר היא צורה נפרדת ולא נעלמת. `__tests__/reverseWinding.test.ts` |
> | O5 — arc flags דחוסים ב-tokenizer | דגל קשת נקרא כתו אחד, כפי שהדקדוק מגדיר (`a1 1 0 011 0`). `__tests__/pathHardening.test.ts` |
> | O6 — פגמי קופי | `quota_exhausted` נכנס ל-`RETRY_POINTLESS`; שגיאת JS גולמית הוחלפה בסוג החריגה בלבד (המלא לקונסולה); `errSchemaOutdated` הפסיק להציג הוראת devops ללקוחה; הפניות אוחדו לרבים (`נסו שוב`, `שלחו`, מדריך המדידה) ו-`termsLabel` ל-`מאשר/ת` |
> | O7 — ה-bind של `geometry-service` | ההערה תוקנה, לא הקישור: הקונטיינר מפורסם ב-`-p 127.0.0.1:8100:8100`, ו-bind ל-`127.0.0.1` בתוכו היה מנתק את הפורט המפורסם |
...
> | Back בדפדפן יוצא מכל המשפך | `pushState` לכל מעבר מסך + `popstate`; הכניסה למשפך לא נלכדת, ו-`processing` אינו יעד |
> | תיבת אישור תנאים בלי קישור לתנאים | קישור ל-`/terms` בתוך התווית, עם `stopPropagation` כדי שקריאה לא תסמן אישור |
> | מספר הרצות בלתי מוגבל על הקופסה | `MAX_CONCURRENT_GENERATIONS` (3). מלא מחזיר `429 BUSY`; בקשה חוזרת על הרצה שבאוויר אינה נדחית — היא כבר נספרה |
> | כליאת פוקוס במודלים (היה O1) | `src/lib/client/useDialog.ts` עושה מיקוד־פנימה, כליאה, החזרה ל-opener ו-Escape; שלושת הדיאלוגים משתמשים בו. **היה רשום כפתוח אחרי שכבר נסגר** |
> | fan-out בלתי מוגבל של פאנלים (היה O3) | `PANELS_PER_RENDER_CAP` (`vectorizer/app/generate.py:38`) חוסם לשלושה פאנלים לכל תא ברשת, ומדווח על מה שנחתך. **היה רשום כפתוח אחרי שכבר נסגר** |
>
> **הלקח, וזו הפעם השנייה שהוא נתפס כאן:** טבלת מצב שנכתבת מהדוח ולא מהקוד
> תהיה שגויה, והכיוון הזה יקר — הוא שולח את הסשן הבא לתקן מה שכבר תוקן, או
> לממש הצעה שכבר נמדדה ונדחתה. משם שתי הטבלאות למעלה: *פתוח* לוכד את הראשון,
> *נדחה* את השני. **שורה בכל אחת מהן עולה grep אחד לפני שכותבים אותה.**
> מסומן: אומת מול הקוד ב-4.8.2026 — כולל הרצת `npm audit`, שהיא הסיבה שמספרי
> O4 השתנו.
>
> ### אחרי המעבר — `aperta-designs.com` (4.8.2026)
>
> סעיפים 7–8 בוצעו. שני דברים שהתגלו רק בייצור, ושניהם תוקנו:
>
> | מה | מה קרה | תיקון |
> |----|--------|-------|
> | isolate שנהרג אחרי שמירת הגרסה | AP-0084: עיצוב נשמר, שורת ה-job נתקעה ב-`saving`, והלקוחה קיבלה "היצירה נכשלה" | `lib/jobRecovery` — הסקר שואל אם ההרצה הפיקה גרסה לפני שהוא מכריז על כישלון |
> | כניסה עם גוגל מ-`www` זרקה לדף הבית | `www` ו-apex שניהם custom domains של ה-Worker, אבל רק apex היה ב-`uri_allow_list`. GoTrue מחליף כתובת חזרה שאינה ברשימה ב-`site_url`; `auth.flow_state` הראה `auth_code_issued_at` בלי session תואם | הרשימה עודכנה, ו-`middleware.ts` מפנה `www` → apex ב-308 |
>
> הלקח מהשני רחב מהכניסה עצמה: **שני origins פירושם שני עותקים של מצב הלקוחה** —
> עוגיות הסשן, הטיוטה ב-`sessionStorage` שנושאת את המשפך לרוחב מסע ה-OAuth,
> ורשימת "העיצובים שלי" ב-`localStorage`. להוסיף כתובת לרשימת ההיתר היה מרפא את
> הסימפטום ומשאיר את הפיצול.
...
> **נסגר 4.8 — והאומדן כאן היה נמוך מדי.** מה שלא נראה מקריאת הקוד הוא
> ש-`Uint8Array.from(atob(s), fn)` מקצה **תו-תו**: מחרוזת היא iterable, ולכן
> `TypedArray.from` בוחרת בפרוטוקול האיטרטור. נמדד על תמונה אחת של 5MB —
> התקרה שהמסך מרשה — לאורך המסלול המלא של `/api/generate`:
>
> | | זמן | שיא heap | מוחזק לאורך ההרצה |
> |---|---|---|---|
> | לפני | 3,005ms | +167MB | 13.4MB |
> | אחרי | 30ms | +5MB | — |
>
> כלומר לא ~60-80MB אלא 167MB, ופעולת פענוח **בודדת** עמדה על 149MB — מעל
> תקרת ה-128MB לבדה. שלושה שינויים: `dataUrlMediaType` קוראת את הכותרת בלי
> לגעת במטען; הצינור קורא רק את `inspiration` הראשונה במקום לעבור על כולן;
> ושורת היומן נושאת את ה-base64 שמוחזק ממילא במקום עותק בייטים שני שהוחזק עד
> סוף ההרצה. הפענוח עצמו עבר ל-`lib/base64.ts` המשותף (גם `runs/persist`
> ו-`text/fonts` נשאו את אותו ניסוח).
>
> **מה שלא נעשה, ובכוונה:** התקרה נשארה 3 תמונות ו-`annotation` נשאר בסכימה.
> אחרי שהפענוח ירד, מטען שלא נקרא כבר לא עולה דבר מלבד הבייטים בגוף הבקשה —
> וההסרה הייתה סוגרת את התפר של שכבת הסימון, שמתועדת כפיצ'ר שממתין לחיבור
> (`AnnotationToolbar.tsx:8`). לחתוך אותו בשם זיכרון שכבר לא נצרך זו עסקה גרועה.
...
(הערות הקובץ מאשרות שצווארי 0.37 מ"מ קורים בפרודקשן — AP-0068.)
...
> **נבדק מול הקוד ב-4.8. שניים מהחמישה תוקנו; לשלושה האחרים ההערכה כאן הייתה
> מוגזמת.** מה שהרשימה למטה לא ידעה הוא שהווקטורייזר עצמו כבר סוגר שניים
> מהפערים במקור: `core/svg_builder.py` מפעיל `orient(poly, sign=1.0)` ופולט כל
> פוליגון כ"מעטפת + חורים" בכיוונים חד-משמעיים, והוא בונה אך ורק `M`/`L`/`Z`.
> כלומר בצינור החי כיוון הליפוף תמיד נכון, ועקומה לא מגיעה למסגור.
>
> | ממצא | מצב |
> |---|---|
> | epsilon בניקוי מול V5 | נסגר קודם (`OPENING_AREA_EPS`) |
> | subpath בכיוון הפוך נעלם | **לא על המסלול החי** — הווקטורייזר עושה `orient`. חשוף רק ל-SVG שלא אנחנו ייצרנו |
> | שני פורמטים "קנוניים" | **תוקן.** `normalizeSvg` פולט את הפוליגונים הדגומים כשה-`d` נושא עקומה; שטוח נשאר מילה במילה |
> | זליגת קואורדינטות בגשרי אות | **תוקן** (`planInFrame`). נמדד בטסט: זליגה של 0.36 מ"מ מול סובלנות 0.5 |
> | ה-tokenizer דוחה arc flags דחוסים | פתוח. אותו שיקול — קלט חוקי שאנחנו לא מייצרים |
>
> **על הגשרים, וזו הנקודה החשובה:** המנגנון אמיתי ונמדד, אבל `matchLetter`
> **מעולם לא ירה בייצור**. 22 הרצות עם כיתוב מתוך 174, רובן בעברית שאין בה
> חלל סגור (`מישל`, `שני` → אפס גשרים מתוכננים), וההרצה היחידה עם 16 גשרים
> מתוכננים (`Aperta`) לא הפיקה אף רשומת `letter`. זו רשת ביטחון שתוקנה לפני
> שנדרשה, לא באג שנצפה.
...
### ~~🟡 בינוני (ביצועים) — `restoreBridges` בלי windowing~~ — נמדד ונדחה
הממצא כפי שנכתב: `src/lib/geometry/restoreBridges.ts:274` סורק לכל אי כל קודקוד
מול כל edge של **כל** החומר — O(V·E), בעוד `thickenBridges` קיבל חלון 2 מ"מ
אחרי שמדד קפיצה מ-186ms ל-1281ms ב-10,000 קודקודים.

**ההצעה נבדקה ונדחתה** — החלון יצא פי 3 גרוע יותר, כי `restoreBridges` סורק אי
מול הגוף הראשי בלבד ואין לו מספיק סריקות לחלוק עליהן את מחיר ה-`intersection`.
המספרים ומקורם: טבלת **נדחה** בראש המסמך. אין להציע את זה שוב בלי מדידה סותרת.
...
- **שם מותג שגוי + הערה פנימית של עורך-דין מוצגים ללקוחות:** התנאים אומרים
  ...
  Aperta, וה-`legalNote` "מומלץ לעבור עם עורך/ת דין" (`he.ts:386`) מוצג בדף
...
ארבעת הפריטים בטבלת **פתוח** בראש המסמך — **O4** (`npm audit`), **O5**
(אי-התאמות גאומטריה, §4), **O6** (קופי, §5), **O7** (ה-bind של
`geometry-service`). אף אחד מהם אינו חוסם, וכולם מתועדים שם עם המיקום והנימוק.
...
נדחה, ויושב בטבלת **נדחה** בראש המסמך כדי שלא יוצע שוב.
