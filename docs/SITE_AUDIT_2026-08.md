...
> ### פתוח
>
> ארבעה, **אף אחד לא חוסם**: כלי פיתוח, שתי אי-התאמות גאומטריה מול קלט שאנחנו
> לא מייצרים, פגמי קופי, והערה לא-נכונה בקוד.
>
> | # | נושא | חומרה | הערה |
> |---|------|-------|------|
...
> | O5 | אי-התאמות גאומטריה — **שתיים מארבע נסגרו 4.8** | 🟢 נמוך | §4. נסגרו: שני פורמטים "קנוניים" סותרים (`normalizeSvg` פולט את הפוליגונים הדגומים כשה-`d` נושא עקומה), וזליגת קואורדינטות בגשרי אות (`planInFrame` — נמדד 0.36 מ"מ מול סובלנות 0.5). נשארו שתיים, ולשתיהן אותו אופי: **קלט SVG חוקי שאנחנו לא מייצרים** — subpath בכיוון ליפוף הפוך, ו-arc flags דחוסים ב-tokenizer. הווקטורייזר עושה `orient` מפורש ופולט `M`/`L`/`Z` בלבד, ולכן הצינור החי לא מגיע לשם |
> | O6 | פגמי קופי ותצוגה | 🟢 נמוך | §5: `quota_exhausted` אומר "נסו שוב מאוחר יותר" ליד כפתור "נסה שוב" (הוא לא ב-`RETRY_POINTLESS`, בניגוד ל-`rate_limited` שכן תוקן), טקסט שגיאת JS גולמי ב-UI, אי-עקביות מגדר |
> | O7 | `geometry-service` נקשר ל-`0.0.0.0` בניגוד להערה שלו | 🟢 קוסמטי | `server.ts:90` מול ההערה בשורה 17 ("מאזין רק על לוקאלהוסט"). האימות שם כבר fail-closed (`503` בלי token) והוא בתוך קונטיינר מאחורי nginx — מה שנשאר הוא ההערה |
>
> ### נדחה — ולמה
>
