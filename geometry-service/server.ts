...
//
// אין לו סודות משלו, אין לו DB, והוא אינו נגיש מהרשת: nginx חושף אותו תחת
// /api/frame על אותו vhost של ה-vectorizer, מאחורי אותו bearer token.
//
// **מי אוכף את זה — ולמה לא ה-bind.** התהליך מאזין על `0.0.0.0`, וזה מכוון:
// הוא רץ בתוך קונטיינר, והקונטיינר מפורסם ב-`-p 127.0.0.1:8100:8100`
// (deploy-geometry.yml). ההגבלה ללוקאלהוסט היא של הפרסום — על הקופסה הפורט
// קשור ללולאה בלבד — ואילו בתוך מרחב הרשת של הקונטיינר `127.0.0.1` הוא כתובת
// אחרת לגמרי: bind אליה היה הופך את הפורט המפורסם ללא-נגיש ומפיל את השירות.
// כלומר "רק לוקאלהוסט" נכון לגבי הקופסה, ולא לגבי השורה שמאזינה.
//
...
// `0.0.0.0` ולא `127.0.0.1` — ראו ההערה בראש הקובץ. בתוך הקונטיינר הלולאה היא
// מרחב אחר, והצמצום אליה מנתק את הפורט המפורסם. הגבול נאכף ב-`-p 127.0.0.1:…`
// ובטוקן, לא כאן.
